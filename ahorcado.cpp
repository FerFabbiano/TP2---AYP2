...
void Ahorcado::comprobarIntento(char* vector, char cadena, int longitud){
	...
	for(int i = 0; i < longitud; i++){
		if(vector[i] == cadena){
			std::cout << "La cadena esta!" << std::endl; // aca habria que hacer un vector de tama̱o (longitud) e ir marcando ahi la posicion donde se encuentra la letra que coincide
			cont += 1;
		}
	}
	if(cont == 0){
